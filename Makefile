...
ifeq ($(OS),Windows_NT)
	...
	ifeq ($(RAYLIB_PATH),)
		...
		_CFLAGS = $(CFLAGS) -I$(RAYLIB_PATH)/include -L$(RAYLIB_PATH)/lib -lraylib -lgdi32 -lwinmm -lws2_32
	endif
else
...
build: src/*
	...
	@mkdir -p bin
	$(CC) src/main.c src/game.c src/net.c src/os/threads.c -o bin/main-build $(_CFLAGS)
...
debug: src/*
	...
	@mkdir -p bin
	$(CC) src/main.c src/game.c src/net.c src/os/threads.c -o bin/main-debug $(_CFLAGS) -DDEBUG
	@echo -e "Running executable ..."
	@bin/main-debug
...
_game.so: src/game.c src/net.c
	...
	@mkdir -p bin
	$(CC) src/game.c -fpic -shared -o bin/game.so $(_CFLAGS) -DHOTRELOADING

watch: src/* _game.so
	@echo -e "Building executable with hot reload mode ..."
	@mkdir -p bin
	$(CC) src/net.c src/os/threads.c -fpic -shared -o bin/net.so $(_CFLAGS)
...
_game.so-debug: src/game.c src/net.c
	...
	@mkdir -p bin
	$(CC) src/game.c -fpic -shared -o bin/game.so $(_CFLAGS) -DHOTRELOADING -DDEBUG
